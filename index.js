        ...
        bootbox.prompt({
            ...
            callback: function(result) {
                if (result === null) {
                    ...
                    let timeOfEdit = timeNow.toGMTString();
                    let newData = { "note": result, "date": originalData.date, "id": originalData.id, "timeEdited": timeOfEdit };
...
clearNotes.addEventListener('click', function(event) {
    if (bootbox.confirm("Are you sure? You can't undo this!")) {
        localStorage.clear();
        location.reload();
    }
})